...
dataName = 'run30_multiple_pxSizes'; % name to identify the particular set of data collected in a particular run
...
deltaAngle = 0.088*5; % increment in razor tilt angle between image capturing
numImages = 5; % number of images to take at each angle, to average over


checkerboardFileNames = ["1.png","2.png","3.png","4.png","6.png"]; % filename of checkerboard to use in this run
filetype = ".png";
...
checkerboards=zeros(length(checkerboardFileNames),800,480);% missing ,3 for colour
for i = 1 : length(checkerboardFileNames)
    ...
    checkerboards(i,:,:) = img; %missing ,: for colour
end
...
for j = 1 : length(checkerboardFileNames)
    ...
    LCDImage.CData = reshape(checkerboards(j,:,:),800,480);% missing ,: and ,3 for colour
    pause(1)
...
while (angle < maxAngle)
    ...
    pause(12); % allow razor to move and vibrations to settle
    ...
    for j = 1 : length(checkerboardFileNames)
        ...
        mkdir(angleFolder);
        LCDImage.CData = reshape(checkerboards(j,:,:),800,480); % missing ,: and ,3
        pause(1)
        for i = 1 : numImages
