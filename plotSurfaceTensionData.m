...
dataName = 'run18_3px_iii\'; % name to identify the particular set of data collected in a particular run
...
pxGrd = 3; % pixels per checker in each dimension
numImages = 5;
filetype = ".png";
...
xlabel("Angle / degrees");
ylabel("Capillary Length / mm");
title("Capillary length against angle","5 images per angle, auto levelled razor, 2px checker, setZeroSkew checked by eye");
saveas(plot3, strcat(plotFolder, "CapVSAngle.fig"));
saveas(plot3, strcat(plotFolder, "CapVSAngle.png"));


curvefit = fittype('a * sinh(x-c)^(2) + b', 'independent', 'x', 'dependent', 'y'); % define fit type 
