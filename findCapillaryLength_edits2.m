...
% Returns the calculated capillary lengths and average capillary length
    ...
    initialCoeffs = [0,-7,0.0,0]; % initial start point for fitting
    ...
    for i = 1:num
        ...
        surfexp = fittype('(exp(((p1*x)+(p2*y))))+sin(p3)*y+C', ...
                            'independent',{'x','y'}, ...
                            'dependent','h');
        ...
        lowerbounds = [-10,-10,-3,-0.1];%[0,-25,0.2,-0.1,-2,-1.5];
        upperbounds = [10,-5,3,0.1];%[2,10,2.0,0.1,3.0,1];
        
        [surffit,gof] = fit([xss,yss],hss,surfexp','Lower',lowerbounds,'StartPoint', initialCoeffs,'Upper',upperbounds)
        
        
        %plot(surffit,[xss(1:250:end),yss(1:250:end)],hss(1:250:end));%,hss(1:1000:end));%mesh(Xmesh(1:20:end,1:20:end),Ymesh(1:20:end,1:20:end),surffit);
